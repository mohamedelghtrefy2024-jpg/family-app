...
export default function ReportsPage() {
  ...
  const paymentsReport = useMemo(() => {
    ...
    const rows = (ctx.payments || [])
      .filter((p) => !p.cancelled && new Date(p.payment_date) >= start && new Date(p.payment_date) <= end)
        ...
        <div>
          {budgetReport.map(({ budget, comparison, scopeLabel }) => (
            <div className="list-row" key={budget.id}>
              <div>
                <div className="list-row-name">{scopeLabel} — {budget.period_type} {budget.period_value}</div>
