...
export default function ExpensesPage() {
  ...
  async function loadExpenses() {
    ...
    setExpenses(all.sort((a, b) => parseLocalDate(b.due_date) - parseLocalDate(a.due_date)));
  }
  ...
  /**
   * BUG-07 FIX: الـ backend (paymentRepo.cancelPayment / paymentService.cancelPayment)
   * كان يعمل بشكل صحيح لكن الواجهة ما كانتش بتوفر زر لتنفيذه. الإلغاء بيحدّث حالة
   * الدفعة فقط (cancelled: true) — بدون حذف السجل، حفاظًا على تاريخ المعاملات.
   * إعادة loadExpenses() بعد الإلغاء تكفي لتحديث outstanding/status/Dashboard/التقارير
   * فورًا لأنها كلها محسوبة دائمًا من computeOutstanding/computeStatus (بند 27) — بدون
   * أي حقل مخزّن يحتاج تحديث يدوي منفصل، وبدون Refresh كامل للصفحة.
   */
  async function handleCancelPayment(expense, payment) {
    setError(null);
    const confirmed = window.confirm(
      `هل أنت متأكد من إلغاء هذه الدفعة؟\n\n` +
      `المصروف: ${expense.description || nameOf(categories, expense.category_id)}\n` +
      `قيمة الدفعة: ${payment.amount}\n` +
      `التاريخ: ${payment.payment_date}`
    );
    if (!confirmed) return;
    try {
      await paymentRepo.cancelPayment(payment.id);
      await loadExpenses();
    } catch (err) {
      setError(err.message);
    }
  ...
  return (
    <div>
      ...
      {activeExpenses.map((expense) => {
        ...
        const cancelledPayments = (payments[expense.id] || []).filter((p) => p.cancelled);
        ...
        return (
          <div className="list-row" key={expense.id} style={{ flexDirection: "column", alignItems: "stretch" }}>
            ...
            {(expensePayments.length > 0 || cancelledPayments.length > 0) && (
              <div style={{ marginTop: 8, paddingTop: 8, borderTop: "1px solid var(--border, #eee)" }}>
                <div className="list-row-meta" style={{ marginBottom: 6 }}>الدفعات</div>
                {expensePayments.map((payment) => (
                  <div
                    key={payment.id}
                    className="list-row"
                    style={{ padding: "6px 0", background: "transparent" }}
                  >
                    <div className="list-row-meta">
                    ...
                    <button
                      ...
                      onClick={() => handleCancelPayment(expense, payment)}
                    >
                      إلغاء الدفعة
                    </button>
                  </div>
                ...
                {cancelledPayments.map((payment) => (
                  <div key={payment.id} className="list-row-meta" style={{ padding: "6px 0", opacity: 0.6 }}>
                    {payment.payment_date} · {payment.amount} · (ملغاة)
                  </div>
                ))}
              </div>
            )}
