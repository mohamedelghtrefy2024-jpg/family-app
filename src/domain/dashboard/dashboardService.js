 ...
 */
import { buildCalendarEvents } from "../calendar/calendarBuilder.js";
import { detectConflicts } from "../scheduling/conflictDetector.js";
import { computeStatus, computeOutstanding } from "../finance/expenseService.js";
import { compareBudget } from "../finance/budgetService.js";
import { parseLocalDate, formatLocalMonth, addDays } from "../shared/dateUtils.js";
...
function buildFinancialSummary(ctx, todayDate) {
  // BUG-05 FIX: كانت monthPrefix تُحسب بـ toISOString() (UTC) — في أول يوم من الشهر
  // بتوقيت القاهرة قرب منتصف الليل كانت ترجع الشهر السابق فتظهر المستحقات صفرًا.
  const monthPrefix = formatLocalMonth(todayDate);
  ...
  const upcomingDue = activeExpenses
    .filter(
      (e) =>
        computeOutstanding(e, ctx.payments || []) > 0 && parseLocalDate(e.due_date) >= todayDate
    )
...
function buildProblems(ctx, todayDate) {
  const rangeEnd = addDays(todayDate, 13); // نافذة أسبوعين قادمين لاكتشاف التعارضات
...
function buildMonthOverview(ctx, todayDate) {
  const monthValue = formatLocalMonth(todayDate);
  const activeExpenses = (ctx.expenses || []).filter((e) => !e.cancelled);
