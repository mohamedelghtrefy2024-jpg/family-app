...
import { buildProblems } from "../dashboard/dashboardService.js";
import { parseLocalDate, formatLocalDate, addDays } from "../shared/dateUtils.js";
...
// BUG-fix (بند 17 — بحث شامل): isoDate/addDays المحليين اتشالوا لصالح dateUtils.js
// المركزي، تفاديًا لنسخة تانية من نفس المنطق (بند 29) ولإصلاح استخدام toISOString (UTC).
function isoDate(d) {
...
function computeNotifications(ctx, todayDate = new Date(), enabledTypes = DEFAULT_ENABLED_TYPES) {
  ...
  if (enabledTypes.includes("lesson_upcoming")) {
    ...
    for (const ev of events) {
      notifications.push({
        ...
        date: ev.date,
        child_id: ev.child_id, // بيانات إضافية مفيدة لو احتجت تفلتر/تجمّع حسب الابن لاحقًا
  ...
  if (enabledTypes.includes("school_start")) {
    for (const year of ctx.academicYears || []) {
      ...
      const start = parseLocalDate(year.start_date);
  ...
  if (enabledTypes.includes("payment_due")) {
    for (const ev of paymentEvents) {
      if (parseLocalDate(ev.date) > addDays(today, 3)) continue;
      notifications.push({
        type: "payment_due",
        date: ev.date,
