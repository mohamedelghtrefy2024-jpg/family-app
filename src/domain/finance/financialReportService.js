...
import { calculateLessonCost } from "../pricing/costCalculator.js";
import { computeOutstanding, computeStatus, computeTotalPaid } from "./expenseService.js";
import { parseLocalDate, formatLocalMonth, addMonths, startOfMonth, endOfMonthEndOfDay } from "../shared/dateUtils.js";
...
function monthBounds(monthValue) {
  ...
  return { start: startOfMonth(anchor), end: endOfMonthEndOfDay(anchor) };
}

/** يزيح "YYYY-MM" بعدد شهور (موجب/سالب) */
function shiftMonth(monthValue, delta) {
  const [y, m] = monthValue.split("-").map(Number);
  return formatLocalMonth(addMonths(new Date(y, m - 1, 1), delta));
}
...
/**
 * BUG-10 FIX: عرض موحّد لفترة الميزانية — يمنع ظهور "Annual — null" للمستخدم.
 * period_value قد يكون null لميزانية سنوية (لأنها تعتمد على scope_ref_academic_year_id
 * بدل period_value)، فنعرض اسم السنة الدراسية بدلًا منه، أو "Annual" فقط لو السنة
 * غير متاحة (بدل عرض null/undefined). يُستخدم في كل من UI وExcel Export (بند 14).
 */
function formatBudgetPeriod(budget, academicYears = []) {
  if (budget.period_type === "Annual") {
    const year = academicYears.find((y) => y.id === budget.scope_ref_academic_year_id);
    return year ? `سنوي — ${year.name}` : "سنوي";
  }
  if (budget.period_type === "Monthly") {
    return `شهري — ${budget.period_value}`;
  }
  return budget.period_value ? `${budget.period_type} — ${budget.period_value}` : budget.period_type;
}
...
function calculateExpectedNextMonth(ctx, referenceDate = new Date()) {
  // BUG-08 FIX: كانت تستخدم referenceDate.toISOString() (UTC) لاستخراج شهر اليوم،
  // فيختلف "الشهر القادم" المحسوب قرب حدود الشهر بتوقيت القاهرة. استُبدلت بـ formatLocalMonth.
  const nextMonthValue = shiftMonth(formatLocalMonth(referenceDate), 1);
  ...
  const last3Start = shiftMonth(formatLocalMonth(referenceDate), -2);
  const { start: rangeStart } = monthBounds(last3Start);
  const recurGroups = new Map();
  for (const e of activeExpenses.filter((e) => !e.is_fixed)) {
    const dueDate = parseLocalDate(e.due_date);
    if (dueDate < rangeStart || dueDate > referenceDate) continue;
    const key = `${e.child_id}|${e.category_id}|${e.description || ""}`;
