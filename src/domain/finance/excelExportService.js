 ...
 */
import { computeStatus, computeOutstanding, computeTotalPaid } from "./expenseService";
import { buildAnnualReport, monthValueOf, classifyCategoryKind, KIND_ORDER } from "./financialReportService";
...
function buildBudgetSheet(XLSX, ctx, compareBudgetFn) {
  ...
  const rows = (ctx.budgets || []).map((b) => {
    ...
    return {
      "النطاق": scopeLabel,
      "الفترة": `${b.period_type} — ${b.period_value}`,
